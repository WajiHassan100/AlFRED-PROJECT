...
from utils.model import LSTMRegressor


DEFAULT_MODEL_PATH = Path(__file__).resolve().parent / "models" / "portfolio_weights_model.pth"
DEFAULT_ONNX_PATH = Path(__file__).resolve().parent / "models" / "portfolio_weights_model.onnx"
...
def build_model(state_dict: dict) -> LSTMRegressor:
    ...
    hidden_layer_size = state_dict["lstm1.weight_hh_l0"].shape[1]
...
def export_onnx(model_path: Path, onnx_path: Path, opset_version: int) -> None:
    ...
    input_size = state_dict["lstm1.weight_ih_l0"].shape[1]
    sample_input = torch.randn(2, 1, input_size, dtype=torch.float32)
    ...
    torch.onnx.export(
        ...
        dynamic_axes={"features": {0: "batch_size"}, "weights": {0: "batch_size"}},
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export Portfolio Rebalancing LSTM model to ONNX.")
